...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
    	...
    	Map<String, Long> groups = stream.parallel().filter(s -> !s.checkIsCurrent()).collect(Collectors.groupingBy(
                s -> s.getFirstName(),
                Collectors.counting()
        ));
    	
    	String result = groups.entrySet().stream().parallel().max((a, b) -> Long.compare(a.getValue(), b.getValue())).map(s -> s.getKey()).orElse("");
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        return (int) stream.parallel().filter(s -> s.getAge() > 20 && s.getGrade() < 65 && !s.checkIsCurrent()).count();
    }
}
